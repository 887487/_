...
window.initAppData = function() {
  ...
  // ヒアリング質問 — HEARING_QUESTIONS_DEFAULT を優先フォールバックとして使用
  if (!window._appCache.hearingQuestions || !window._appCache.hearingQuestions.length) {
    if (HEARING_QUESTIONS_DEFAULT && HEARING_QUESTIONS_DEFAULT.length) {
...
function _hrGetQuestions() {
  var fromCache = _hrQuestionsLoad();
  if (fromCache && fromCache.length) return fromCache;
  // キャッシュが空の場合は HEARING_QUESTIONS_DEFAULT を直接使う
