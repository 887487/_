* ヒアリング結果が枠内に収まっていない（結果のテキストが重複。）
* [📕 マニュアル]に表示を変更して、アコーディオンの▼にボタン枠の末尾を合わせる
* 画面遷移に画像が0件のパターンは表示しない
* 画面遷移のパターン名クリックで先頭画像をプレビュー
* 画面遷移上のツールチップの表示サイズ…固定する（画像の表示サイズに依存しない）
* トークスクリプト上の表の列幅を均等にする
* ---
