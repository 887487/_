...
20260720追記
* メールデータをExcelから読み込む
* メールテンプレに有効/無効のトグルで最新版のみ表示する

---
20260807追記
* videosUrl 追加　→　保存して反映　→　画面更新でvideoUrlが消滅する（各ページのサイドメニューにもボタン表示なし）
* 並び順を 0→9,a→z,あ→お で固定する
* ダークモードの配色を改善する
* screen.html パターン名クリック　先頭画像へのジャンプを廃止して、アコーディオンクローズに変更
* index.html に お知らせ の表示枠を追加（表示内容は ./★お知らせ★.txt から自動読み込み）
* admin.html 画面遷移タブ 画像セット化廃止
