...
function _smFileIcon(path) {
  var ext = (path || '').split('.').pop().toLowerCase();
  if (/^xlsx?$/.test(ext) || ext === 'csv' || ext === 'ods') return '📊';
  if (ext === 'pdf') return '📕';
    ...
    if (sec.type === 'links_with_sub') {
      ...
      (sec.subSections || []).forEach(function(sub, sj) {
        ...
        var lis = (sub.items || []).map(function(it) {
          ...
          if (it.file) {
            var _ext1 = (it.file||'').split('.').pop().toLowerCase();
            var _ico1 = _smFileIcon(it.file);
            var _fname1 = (it.file||'').replace(/.*[/\\]/, '');
            return '<li><a href="' + (it.file||'#') + '" download="' + _fname1 + '" style="display:flex;align-items:center;gap:4px;">' + _ico1 + ' ' + it.name + '<span style="font-size:9px;background:var(--surface2);border:1px solid var(--border);border-radius:3px;padding:0 3px;color:var(--text3);margin-left:4px;text-transform:uppercase;">' + _ext1 + '</span></a></li>';
          }
          if (it.fileId) {
      ...
      var lis = (sec.items || []).map(function(it) {
        ...
        if (it.file) {
          var _ext2 = (it.file||'').split('.').pop().toLowerCase();
          var _ico2 = _smFileIcon(it.file);
          var _fname2 = (it.file||'').replace(/.*[/\\]/, '');
          return '<li style="display:flex;align-items:center;gap:4px;"><a href="' + (it.file||'#') + '" download="' + _fname2 + '" style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + _ico2 + ' ' + it.name + '</a><span style="font-size:9px;background:var(--surface2);border:1px solid var(--border);border-radius:3px;padding:0 3px;color:var(--text3);flex-shrink:0;text-transform:uppercase;">' + _ext2 + '</span></li>';
        }
        if (it.fileId) {
          return '<li><a href="javascript:void(0)" onclick="window._smFileAction(\'' + it.fileId.replace(/'/g,"\\'") + '\')" style="display:flex;align-items:center;gap:4px;">📎 ' + it.name + '</a></li>';
        }
